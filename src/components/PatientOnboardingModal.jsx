...
/**
 * Deterministically generates a fresh 5-digit Patient ID from an Auth0 sub or random entropy
 */
...
export default function PatientOnboardingModal({ user, onComplete }) {
  // Pre-fill Name if available from Auth0 (editable)
  const initialName = user?.name || user?.nickname || "";
  ...
  const handleSubmit = (e) => {
    ...
    // Save profile locally tied to Auth0 sub
    if (user?.sub) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="w-full max-w-lg rounded-2xl p-6 sm:p-8 border bg-white dark:bg-zinc-900 border-slate-200/90 dark:border-zinc-800 shadow-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Name Field (Pre-filled from Auth0 if available, editable) */}
          <div>
          ...
          {/* Email Field (Pre-filled from Auth0, read-only display) */}
          {user?.email && (
            <div>
              ...
              <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-slate-200/60 dark:border-zinc-800 bg-slate-100/60 dark:bg-zinc-800/40 text-slate-500 dark:text-zinc-400 text-xs sm:text-sm">
                <Mail className="w-4 h-4 text-slate-400 shrink-0" />
                <span className="truncate">{user.email}</span>
                <span className="ml-auto text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800">
                  Verified
                </span>
              </div>
            </div>
